...
function updateEmployeeStats(employees) {
    const stats = employees.reduce((acc, emp) => {
        ...
        acc[emp.status]++;
        return acc;
    }, { total: 0, active: 0, inactive: 0, on_leave: 0 });
    ...
    document.getElementById('onLeaveEmployees').textContent = stats.on_leave;
    document.getElementById('inactiveEmployees').textContent = stats.inactive;
}
